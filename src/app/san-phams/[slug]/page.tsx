...
const ProductPage = ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  // Trong thực tế, dữ liệu này sẽ được lấy từ API dựa trên slug
          ...
          <div className="border-t pt-6">
            ...
            <div className="prose prose-green">
              {displayedDescription.map((line, index) => (
                <p key={index} className="mb-2">{line}</p> // Render lines
              ))}
              {descriptionLines.length > initialLinesToShow && ( // Show button only if there are more lines
                <button
